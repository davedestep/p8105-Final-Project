...
```{r}
model_data$time <- as.yearmon(paste(model_data$year, model_data$month), "%Y %m")

# Sys.getlocale("LC_TIME")
# 
# model_data<-model_data %>%
#    unite(time, c(month, year), sep = "-", remove = FALSE) %>%
#    mutate(time = parse_date_time(time, "my"), local="English_United States.1252")

```



## Time Series of AMI Hospitalizations Grouping by Gender
```{r}
model_data %>%  
...
model_data %>%  
  ...
  geom_point()+
  scale_color_viridis(option = "B")+
  ggtitle("Number of MI's by PM10 and Year/Month")
```
...
model_data %>%  
  ...
  geom_point()+
  scale_color_viridis(option = "B")+
  ggtitle("Number of MI's by NO2 and Year/Month")
```
...
count3 %>%
  ggplot(aes(x = n)) + geom_histogram(aes(color = "red")) + labs(title = "Distribution of Counts of MI Data")
```
The counts appear to be normally distributed, no overdispersion is present.
...
## Should we not include temp? Most is significant when not included...
```{r}
...
## Including temp?
```{r}
summary(m1 <- glm(n ~ avg_pm2_5 + avg_pm10 + avg_co + avg_no2 + avg_temp + gender, family="poisson", data=model_data))
...
summary(m1 <- glm(n ~ avg_pm2_5 + avg_pm10 + avg_co + avg_no2 + month + year + gender, family="poisson", data=model_data))
...
knitr::kable(tab, digits = 3)
```
Talk about the effect of including month and year

## Including both temp and months/years?
```{r}
summary(m1 <- glm(n ~ avg_pm2_5 + avg_pm10 + avg_co + avg_no2 + month + year +avg_temp + gender, family="poisson", data=model_data))
...
knitr::kable(tab, digits = 3)
```
Interestingly, all of the variables are significant here.
...
summary(m1 <- glm(n ~ avg_pm2_5 + avg_pm10 + avg_co + avg_no2 + month + year +avg_temp + gender, family="poisson", data=model_data2))
...
knitr::kable(tab, digits = 6)
```
Now all of the variables are significantly associated with MI.
